...
funDef='b1*exp(-b2*x) + b3*exp(-b4*x) + b5*exp(-b6*x)'; 
dfunDef='[exp(-b2*x),-b1*x.*exp(-b2*x),exp(-b4*x),-b3*x.*exp(-b4*x),exp(-b6*x),-b5*x.*exp(-b6*x)]';
pDef='b1 b2 b3 b4 b5 b6'; Names='y x';
deff('yhat=Lanczos3(b,x)','yhat=b(1)*exp(-b(2)*x) + b(3)*exp(-b(4)*x) + b(5)*exp(-b(6)*x)');
//
//
//
...
//
bInit1=[1.2 0.3 5.6 5.5 6.5 7.6]';
bInit2=[0.5 0.7 3.6 4.2 4 6.3]';
b=[8.6816414977E-02;9.5498101505E-01;8.4400777463E-01;2.9515951832E+00;1.5825685901E+00;4.9863565084E+00];
resSS=1.6117193594E-08;
