...
funDef='b1*exp(-b2*x) + b3*exp(-(x-b4).^2 ./ b5.^2 ) + b6*exp(-(x-b7).^2 ./ b8.^2 )'; 
dfunDef='[exp(-b2*x),-b1*x.*exp(-b2*x),exp(-((x-b4)/b5).^2),2*b3*(x-b4).*exp(-((x-b4)/b5).^2)/(b5^2),2*b3*(x-b4).^2/(b5^3).*exp(-((x-b4)/b5).^2),exp(-((x-b7)/b8).^2),2*b6*(x-b7).*exp(-((x-b7)/b8).^2)/b8^2,2*b6*(x-b7).^(2).*exp(-((x-b7)/b8).^2)/b8^3]';
pDef='b1 b2 b3 b4 b5 b6 b7 b8'; Names='y x';
deff('yhat=Gauss1(b,x)','yhat=b(1)*exp(-b(2)*x) + b(3)*exp(-(x-b(4)).^2 ./ b(5).^2 ) + b(6)*exp(-(x-b(7)).^2 ./ b(8).^2)');
// 
// 
...
//
bInit1=[87 .009 100 65 20 70 178 16.5]';
bInit2=[94 .0105 99 63 25 71 180 20]';
b=[9.8778210871E+01;1.0497276517E-02;1.0048990633E+02;6.7481111276E+01;2.3129773360E+01;7.1994503004E+01;1.7899805021E+02;1.8389389025E+01];
resSS=1.3158222432E+03;
