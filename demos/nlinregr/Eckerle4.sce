...
funDef='(b1/b2)*exp(-0.5*((x-b3)./b2).^2)'; pDef='b1 b2 b3'; Names='y x';
dfunDef='[exp(-((x-b3).^2)./(2*b2^2))/b2,b1/b2^2*exp(-(x-b3).^2/(2*b2^2)).*((x-b3)^2/b2^2-1),b1*(x-b3)/(b2^3).*exp(-(x-b3).^2/(2*b2^2))]';
deff('yhat=Eckerle4(b,x)','yhat=(b(1)/b(2))*exp(-0.5*((x-b(3))/b(2)).^2)');
//
//
//
...
bInit1=[1 10 500]';
bInit2=[1.5 5 450]';
b=[1.5543827178E+00;4.0888321754E+00;4.5154121844E+02];
resSS=1.4635887487E-03;
