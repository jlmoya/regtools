...
funDef='b1 ./ (1+exp(b2-b3*x))'; pDef='b1 b2 b3'; Names='y x';
dfunDef='[ones(x)./(1+exp(b2-b3*x)),-b1*exp(b2-b3*x)./(exp(b2-b3*x)+1).^2,b1*x.*exp(b2-b3*x)./(exp(b2-b3*x)+1).^2]';
deff('yhat=Rat42(b,x)','yhat=b(1) ./ (1+exp(b(2)-b(3)*x))');
//
//
//
...
bInit1=[100 1 .1]';
bInit2=[75 2.5 0.07]';
b=[7.2462237576E+01;2.6180768402E+00;6.7359200066E-02];
resSS=8.0565229338E+00;
